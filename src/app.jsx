...
export default function PrayerApp() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: CREAM, fontFamily: "'Georgia', 'Times New Roman', serif" }}>
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, backgroundImage: `radial-gradient(ellipse at 15% 10%, rgba(244,184,200,0.18) 0%, transparent 50%), radial-gradient(ellipse at 85% 90%, rgba(244,184,200,0.12) 0%, transparent 50%)` }} />
      <header style={{ position: "relative", zIndex: 1, textAlign: "center", padding: "48px 24px 32px", borderBottom: "1px solid rgba(244,184,200,0.3)", background: "linear-gradient(180deg, #fff 0%, #fdf8fb 50%, #f7f5f0 100%)" }}>
        ...
        <h1 style={{ fontSize: "clamp(32px, 5vw, 52px)", fontWeight: "400", color: DARK, margin: "0 0 8px" }}>Lifted by Prayer</h1>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 10, display: "flex", justifyContent: "center", background: "#fff", borderBottom: "1px solid rgba(244,184,200,0.3)", boxShadow: "0 2px 16px rgba(244,184,200,0.15)" }}>
        {[{ key: "wall", label: "Prayer Wall" }, { key: "submit", label: "Submit Request" }, { key: "donate", label: "Give" }].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{ padding: "16px 24px", border: "none", background: "none", cursor: "pointer", fontSize: "12px", letterSpacing: "2px", textTransform: "uppercase", fontFamily: "sans-serif", color: tab === t.key ? DARK : "#a89880", borderBottom: tab === t.key ? `2px solid ${PINK_DARK}` : "2px solid transparent", fontWeight: tab === t.key ? "700" : "400", transition: "all 0.2s" }}>{t.label}</button>
        ))}
      </nav>
          ...
          <div>
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "32px", justifyContent: "center" }}>
              {["All", ...categories].map(c => (
                <button key={c} onClick={() => setFilterCategory(c)} style={{ padding: "6px 16px", border: `1px solid ${filterCategory === c ? PINK_DARK : "#e0d8cc"}`, borderRadius: "20px", background: filterCategory === c ? PINK_DARK : "transparent", color: filterCategory === c ? "#fff" : MUTED, fontSize: "12px", letterSpacing: "1px", cursor: "pointer", fontFamily: "sans-serif", transition: "all 0.2s" }}>{c}</button>
              ))}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              {filtered.map(prayer => (
                <div key={prayer.id} style={{ background: "#fff", borderRadius: "14px", padding: "28px", boxShadow: "0 2px 20px rgba(244,184,200,0.15)", border: "1px solid rgba(244,184,200,0.2)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "14px", flexWrap: "wrap", gap: "8px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                      <div style={{ width: "38px", height: "38px", borderRadius: "50%", background: `linear-gradient(135deg, ${PINK_LIGHT}, #faf6ef)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", border: `1px solid ${PINK}44` }}>{prayer.anonymous ? "🙏" : prayer.name[0]}</div>
                      <div>
                    ...
                    <span style={{ padding: "4px 12px", borderRadius: "12px", fontSize: "11px", letterSpacing: "1px", background: `${categoryColors[prayer.category]}22`, color: categoryColors[prayer.category], fontFamily: "sans-serif", fontWeight: "600", textTransform: "uppercase" }}>{prayer.category}</span>
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                      ...
                      <button onClick={() => toggleComments(prayer.id)} style={{ background: "none", border: "none", color: PINK_DARK, cursor: "pointer", fontSize: "13px", fontFamily: "sans-serif", padding: "0" }}>💬 {prayer.comments.length} {prayer.comments.length === 1 ? "comment" : "comments"}</button>
                    </div>
                    <button onClick={() => handlePray(prayer.id)} style={{ padding: "8px 22px", border: `1px solid ${prayedFor[prayer.id] ? PINK_DARK : PINK}`, borderRadius: "20px", background: prayedFor[prayer.id] ? `linear-gradient(135deg, ${PINK}, ${PINK_DARK})` : PINK_LIGHT, color: prayedFor[prayer.id] ? "#fff" : PINK_DARK, fontSize: "12px", letterSpacing: "1.5px", cursor: prayedFor[prayer.id] ? "default" : "pointer", fontFamily: "sans-serif", fontWeight: "700", textTransform: "uppercase", transition: "all 0.3s" }}>{prayedFor[prayer.id] ? "✓ Prayed" : "Pray"}</button>
                  </div>
